...
function HeroCard({ data, skills }: { data: Information; skills: { [key: string]: Skill } }) {
  return (
    <div className="bg-zinc-800 rounded-lg border-stone-400 border px-3 py-5 flex flex-col gap-3">
      <div className="border-b-stone-100 border border-transparent pb-2">
        <h1 className="mb-2 text-xl font-bold text-stone-100">{data.name}</h1>
        <h2 className="w-[190px] text-sm text-stone-100">{data.description}</h2>
      </div>
      ...
      <div className="flex justify-between">
        <a target="_blank" href={data.link} className="px-3 py-[0.1rem] rounded-md border-white bg-green-600 font-semibold">See Live</a>
